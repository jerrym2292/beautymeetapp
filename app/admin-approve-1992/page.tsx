...
export default function AdminApprovePage() {
  ...
  const canLoad = useMemo(() => unlocked && pin.length > 0, [unlocked, pin]);

  useEffect(() => {
    const saved = localStorage.getItem("beautymeet_admin_pin");
    if (saved) {
      setPin(saved);
    }
  }, []);
  ...
  async function load() {
    setLoading(true);
    setErr(null);
    const res = await fetch(`/api/admin/applications?pin=${encodeURIComponent(pin)}`);
  ...
  async function approve(appId: string) {
    ...
    const baseZip = prompt("Provider base ZIP (Atlanta area):") || "";
    if (!baseZip) return;

    const res = await fetch(`/api/admin/approve`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ pin, appId, displayName, baseZip }),
    });
  ...
  async function reject(appId: string) {
    ...
    const res = await fetch(`/api/admin/reject`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ pin, appId, notes }),
    });
  ...
  return (
    <main>
      ...
      {!unlocked ? (
        <section style={card}>
          <div style={{ fontWeight: 800, marginBottom: 8 }}>Enter PIN</div>
          <input
            value={pin}
          ...
          <button
            onClick={() => {
              localStorage.setItem("beautymeet_admin_pin", pin);
              setUnlocked(true);
            }}
            style={buttonStyle}
          >
          ...
          <div style={{ fontSize: 12, opacity: 0.7, marginTop: 10 }}>
            MVP-only: PIN is checked server-side on each admin request.
          </div>
        </section>
      ) : (
        <section style={card}>
            ...
            {apps.map((a) => (
              <div key={a.id} style={{ ...card, marginTop: 0 }}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                  ...
                  <button
                    onClick={() => reject(a.id)}
                    style={{ ...buttonStyle, flex: 1, borderColor: "rgba(248,113,113,0.5)", background: "rgba(248,113,113,0.12)" }}
                  >
                    Reject
                  </button>
                </div>
